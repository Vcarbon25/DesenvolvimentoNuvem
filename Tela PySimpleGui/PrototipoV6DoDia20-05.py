...
while True:
    ...
    if HabilitarCamera==True:
        ...
        if escolha == 'Camera Simples':
            cap = cv2.VideoCapture(CamIndex)
            sucess, cv2img = cap.read()
            #principal.FindElement('camera').Update(data=cv2.imencode('.png', cap.read()[1])[1].tobytes())            print('lugarcerto')
            #principal.FindElement('camera').Update(data=imgtela)
            principal.FindElement('camera').Update(data=cv2.imencode('.png', cv2img)[1].tobytes())
            cap.release()
